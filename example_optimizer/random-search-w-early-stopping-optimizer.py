...
class RandomSearchWEarlyStoppingOptimizer(AbstractOptimizer):
    ...
    def observe(self, x: List[List[int]], y: List[List[float]]) -> NoReturn:
        ...
        for _y in y:
            self.y.append(_y)
        """
            NOTICE: `self.early_stopping` can be modified to support the
            early stopping criterion.
            We only use a very naive way to implement the early stopping
            criterion just for demonstration only.
        """
