...
"""
    Gaussian process optimizer constructs a gaussian process model for the
...
class GaussianProcessRegressorOptimizer(AbstractOptimizer):
    ...
    def __init__(
        ...
        # we set `n_suggestions` to be 5 as a demonstration example.
    ...
    def suggest(self) -> List[List[int]]:
        ...
        """
            NOTICE: we can also use the model to sweep the design space
            if the design space is not quite large.
            Specifically, we only use a very naive way to pick up the design
            just only for demonstration.
        """
        ...
        for point in potential_parteo_frontier:
            try:
                ...
                # randomly select a potential suggest
                _potential_suggest.append(random.sample(potential_suggest, k=1)[0])
        # remove duplicated suggestions
        _potential_suggest = [list(__x) for __x in {tuple(_x) for _x in _potential_suggest}]
